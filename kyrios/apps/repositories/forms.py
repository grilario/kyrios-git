...
from .models import Repository, REPOSITORY_NAME_MAX_LENGTH, REPOSITORY_NAME_MIN_LENGTH, REPOSITORY_DESCRIPTION_MAX_LENGTH
...
def name_length_validator(value):
    """
        Validates given name in terms of length and uniqueness among user's repositories.
    """
...
def ascii_validator(value):
    ...
    try:
        ...
        raise forms.ValidationError(
            u'Repository name should be in English characters.')
...
def special_characters_validator(value):
    ...
    if set('!@#$%^&*.()+={}[]:;"\'\/<>,?').intersection(value):
        raise forms.ValidationError(
            u'Repository name should only contain letters, numbers, hyphen and underscore.')
...
class RepositoryCreationForm(forms.Form):
    ...
    name = forms.CharField(required=True, validators=[
                           name_length_validator, ascii_validator, special_characters_validator])
    ...
    def __init__(self, *args, **kwargs):
        ...
        # boolean flag to indicate we're not creating a new object.
    ...
    def _initialize_form(self, instance):
        ...
        if instance is not None:
            if not isinstance(instance, Repository):
                raise TypeError('Instance should be of Repository type.')
            self.instance = instance
            # store current repository name in case user changed it's name.
            self.repo_entering_name = instance.name
    ...
    def _repo_exists(self, repository_name):
        ...
        user_repos = Repository.objects.all_repositories(user=self.user)
        ...
        # if object is being edited we check for a name conflict with other repos.
        elif self.edit and self.instance is not None:
    ...
    def _update_instance(self, data):
        ...
        if self.instance is None:
            raise ValueError('Instance object shoud not be None.')
    ...
    def clean(self):
        ...
        if self._repo_exists(name):
            raise forms.ValidationError(
                u'You have another repository with this name.')
    ...
    def save(self):
        ...
        if self.edit:
            ...
            # rename repository folder if it's name has changed.
            if self.instance.name != self.repo_entering_name:
