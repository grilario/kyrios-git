...
from .decorators import require_existing_repo, require_existing_rev, require_access, require_owner_access
from .forms import RepositoryCreationForm, RepositoryArea51Form
...
def new_repository(request):
    ...
    return render(request, 'repositories/new.html', {'form': form})
...
def repository_commits(request, username, repository):
    ...
    requested_repo = Repo(Repo.get_repository_location(username, repository))
    commits = {branch: requested_repo.get_commits(
        branch) for branch in requested_repo.get_branches()}
    return render(request, 'repository/repo-pjax.html',
...
def repository_graphs(request, username, repository):
    ...
    return render(request, 'repository/repo-pjax.html', context)
...
def repository_settings(request, username, repository):
    ...
    if request.method == 'POST':
        form = RepositoryCreationForm(
            data=request.POST, user=request.user, instance=repo, edit=True)
        if form.is_valid():
            form.save()
